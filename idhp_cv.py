...
for i in tqdm(range(10)):
    ...
    for train_idxs, test_idxs in kf.split(ihdp.values):
        ...
        mse_s_cv[j],_ = s_learner(xs, treats, outcomes, xs_test, cates_test)
        mse_t_cv[j],_ = t_learner(xs, treats, outcomes, xs_test, cates_test)
        mse_r_cv[j],_ = r_learner(xs, treats, outcomes, xs_test, cates_test)
        mse_dr_cv[j],_ = dr_learner(xs, treats, outcomes, xs_test, cates_test)
        j+=1
